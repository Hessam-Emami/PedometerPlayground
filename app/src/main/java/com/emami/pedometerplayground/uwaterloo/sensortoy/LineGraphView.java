...
package com.emami.pedometerplayground.uwaterloo.sensortoy;
...
public class LineGraphView extends View
{
    ...
    private void prepForData(Canvas canvas)
    {
        ...
        graphPaint.setTextSize(TEXT_SIZE);

        canvas.drawText(Float.toString(maxY) + " m/s^2", 10, TEXT_SIZE, graphPaint);
        canvas.drawText("-" + Float.toString(maxY) + " m/s^2", 10, HEIGHT - 10, graphPaint);

        for(int i = 0; i < labels.size(); i++){
            canvas.drawText(labels.get(i) + ":", 10, 2*TEXT_SIZE + i*(TEXT_SIZE + 20), graphPaint);
            canvas.drawLine(10, 2*TEXT_SIZE + i*(TEXT_SIZE + 20), AXIS_WIDTH - 20, 2*TEXT_SIZE + i*(TEXT_SIZE + 20), linePaints.get(i));
        }
    }
